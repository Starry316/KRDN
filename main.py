...
def get_feed_data(train_entity_pairs, train_user_set):
    def negative_sampling(user_item, train_user_set):
        ...
        for i in range(user_item.shape[0]):
            neg_items.append(get_neg_one(user_item.cpu().numpy()[i, 0]))
...
for epoch in range(args.epoch):
    ...
    STLogger.info('train done here')
    STLogger.info(f'saving model to {args.out_dir}')
    mkdir(args.out_dir)
    saveModel(os.path.join(args.out_dir, f'model_{args.dataset}.ckpt'), model, optimizer, None, epoch=epoch)
